...
- `v4.5.60 (2026-04-01)` 正式 Release 对齐与远端发布加固版：不改写既有 `v4.5.59` 历史，改用新的 `v4.5.60` 对齐干净源码正式发布；批量滚动脚本补齐 `expect / mktemp / 远端 bootstrap` 实战修复，并统一默认版本口径到 `v4.5.60`。
- `v4.5.59 (2026-04-01)` Docker 发布续航、默认镜像对齐与双机批量滚动升级版：统一部署模板和安装/更新脚本默认镜像到 `v4.5.59`，让 Docker 发布脚本支持复用现有登录态，并新增可批量滚动两台服务器多实例的发布脚本。
- `v4.5.58 (2026-03-31)` 农场批量作业导航与 QQ 高风险保存预览增强版：农场批量模式新增分组快选、覆盖率提示、底部操作坞与批量执行高亮反馈；背包详情弹窗补齐 Esc/焦点与滚动锁定；设置页新增 QQ 高风险保存前风险预览，并统一当前版本口径到 `v4.5.58`。
...
### 开发补记 - v4.5.60 正式 Release 对齐与远端发布加固版 (2026-04-01)

#### ✅ 本轮正式 Release 对齐
- ✅ **本轮明确改用新版本而不是改写既有 `v4.5.59` tag**: 这样 GitHub Release 页面、源码归档、`git checkout v4.5.60` 和当前干净 `main` 会继续保持一一对应。
- ✅ **当前默认版本已统一切到 `v4.5.60`**: `core/package.json`、`web/package.json`、部署模板、离线包脚本、GitHub Actions 默认版本和 README 示例命令已同步抬升。

#### ✅ 本轮远端滚动脚本加固
- ✅ **`publish-and-rollout.sh` 已补齐 `expect` 密码匹配与 macOS `mktemp` 兼容修复**: 真实执行双机滚动时暴露出来的本地入口问题已经纳入脚本本身，不再需要靠临时手工绕过。
- ✅ **远端 helper 现在优先复用服务器现有 `install-or-update.sh`**: 只有远端确实没有脚本束时才下载临时副本，可降低 GitHub Raw 抖动导致的下载不完整和执行中断风险。

#### ✅ 本轮 Release / 核验口径
- ✅ **Release 附件巡检默认范围已扩到 `v4.5.60`**: `check-release-assets.js` 现在会默认检查 `v4.5.54 ~ v4.5.60`。
- ✅ **本轮版本用于承接“正式发布口径 + 双机验收结果”**: 后续 GitHub Release、Docker 镜像、README 和服务器升级结果会继续以 `v4.5.60` 为共同基线。

### 开发补记 - v4.5.59 Docker 发布续航、默认镜像对齐与双机批量滚动升级版 (2026-04-01)

#### ✅ 本轮发布链路补强
- ✅ **`auto-update-docker.sh` 现在支持复用当前 Docker / GHCR 登录态**: 本机已经执行过 `docker login` 时，不必每次重新导出 Token；如显式提供 `DOCKERHUB_TOKEN / GHCR_TOKEN`，脚本仍会优先使用 Token 登录。
- ✅ **新增 `publish-and-rollout.sh` 统一发布与远端滚动入口**: 本地完成镜像推送后，可以按 `host|user|password_env|action|stack_name|current_link|web_port` 目标列表继续执行远端安装/升级，减少两台服务器多栈手工重复操作。

#### ✅ 本轮版本与模板统一
- ✅ **当前默认版本已统一切到 `v4.5.59`**: `core/package.json`、`web/package.json`、部署模板、离线包脚本、GitHub Actions 默认版本和 README 示例版本已同步抬升。
- ✅ **旧模板回落问题已修复**: `deploy/docker-compose.yml` 之前仍停在 `4.5.55`，新装机器可能误装旧镜像；本轮已经和 `.env.example / fresh-install / update-app / verify-stack / update-agent` 一起收口到 `v4.5.59`。

#### ✅ 本轮文档与核验口径
- ✅ **README、部署文档与帮助中心已补齐最短实操命令**: 现在可以直接照着文档执行 Docker Compose 首装、镜像发布和双机多实例滚动升级。
- ✅ **Release 附件巡检默认范围已更新到最近版本**: `check-release-assets.js` 默认会检查 `v4.5.54 ~ v4.5.59`，更贴近当前真实发布节奏。

### 开发补记 - v4.5.58 农场批量作业导航与 QQ 高风险保存预览增强版 (2026-03-31)

#### ✅ 本轮农场批量工作台增强
- ✅ **批量模式新增“分组快选 + 覆盖率摘要 + 颜色图例 + 底部操作坞”**: `FarmPanel.vue` 现在可按可收获/待浇水/待除草/待除虫直接选区，并实时显示当前筛选范围的可处理覆盖率，批量执行入口固定在底部，不再需要频繁回到上方工具区。
- ✅ **批量执行反馈升级为“可追踪结果流”**: 新增选中数量脉冲提示、执行结果自动淡出和“最近处理地块”高亮回显，处理后可以一眼确认哪些地已经执行，不再只靠 toast 记忆。
- ✅ **土地卡片补齐批量语义标签和色带**: `LandCard.vue` 在批量模式下新增待办标签（可收获/待浇水/待除草/待除虫/观察中）与对应语义色带，并支持局部成功闪烁反馈。

#### ✅ 本轮背包详情弹窗稳态补强
- ✅ **详情弹窗补齐键盘可达与焦点回归**: `BagPanel.vue` 现在支持 Esc 关闭、打开后自动聚焦详情面板、关闭后回到先前焦点节点，键盘连续操作更稳。
- ✅ **弹窗结构与交互统一收口**: 弹窗改为 shell + backdrop 双层结构，支持点击背景关闭、关闭按钮事件收口，并在打开时锁定 `body` 滚动，避免背景穿透滚动。

#### ✅ 本轮 QQ 高风险保存预览增强
- ✅ **保存前风险预览已接入三个关键保存入口**: `Settings.vue` 会在页底保存区、账号保存悬浮坞和差异确认弹窗统一展示“本次保存会发生什么”的预览卡片。
- ✅ **预览策略按真实风险场景动态生成**: 会根据“是否新开启高风险项”“是否切换到不限制模式”“当前高风险项总数”等条件给出不同语义提示，显式区分“临时窗口”和“手动收口模式”。
- ✅ **不限制模式视觉强化已补齐**: 工具条、模式切换按钮和状态胶囊新增危险态样式与过渡动画，保存前的风险识别更直观。

#### ✅ 本轮发布口径
- ✅ **当前默认版本已统一切到 `v4.5.58`**: `core/package.json`、`web/package.json`、README、部署模板、Docker 发布脚本、GitHub Actions 默认版本与帮助中心 Release Notes 已同步抬升。
- ✅ **本轮继续承接 GitHub / Docker / 服务器升级主线**: 新版本将用于 GitHub Releases、Docker Hub 镜像、更新公告追加与多端口部署验证。
