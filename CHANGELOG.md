...
- `v4.5.59 (2026-04-01)` Docker 发布续航、默认镜像对齐与双机批量滚动升级版：统一部署模板和安装/更新脚本里的默认镜像到 `v4.5.59`，让 `auto-update-docker.sh` 支持复用现有 Docker 登录态，并新增可批量滚动两台服务器多实例的发布脚本。
- `v4.5.58 (2026-03-31)` 农场批量作业导航与 QQ 高风险保存预览增强版：农场批量模式新增分组快选、覆盖率提示、底部操作坞与批量执行高亮反馈；背包详情弹窗补齐 Esc/焦点与滚动锁定；设置页新增 QQ 高风险保存前风险预览，并统一当前发布口径到 `v4.5.58`。
...
### 🏷️ 正式 Release 改用新版本对齐干净源码
- **不再改写已发布的 `v4.5.59` 历史**: GitHub 上已有的 `v4.5.59` tag / Release 保持不动，这次改为新发 `v4.5.60`，让 Release 页面、源码归档、`git checkout v4.5.60` 和当前干净 `main` 保持一致。
- **默认版本入口统一抬升到 `v4.5.60`**: `core/package.json`、`web/package.json`、部署模板、离线包脚本、GitHub Actions 默认版本、README 示例命令和帮助中心发布索引都已同步切到 `v4.5.60`。

### 🚀 远端批量发布脚本补齐实战修复
- **`publish-and-rollout.sh` 补齐 `expect` 与 `mktemp` 兼容性修复**: 修复了密码提示匹配在 Tcl/expect 下的解析问题，以及 macOS 临时脚本文件名模板不兼容导致的首轮执行失败。
- **远端安装/更新改为优先复用服务器现有脚本束**: `install` / `update` 现在会优先使用远端已有的 `install-or-update.sh`，只有缺失时才下载临时副本，降低 GitHub Raw 网络波动造成的半截脚本执行风险。

### 📦 Release 与核验口径继续收口
- **最近版本巡检范围扩到 `v4.5.60`**: `scripts/utils/check-release-assets.js` 默认会继续检查 `v4.5.54 ~ v4.5.60`，方便沿着最新正式版本继续核对附件完整性。
- **这轮版本作为“正式发布口径”和“服务器验收口径”的统一基线**: 便于后续 Docker 镜像、GitHub Release、README 安装示例与双机部署结果继续按同一版本号追踪。

## [v4.5.59] - Docker 发布续航、默认镜像对齐与双机批量滚动升级版 (2026-04-01)
### 🐳 Docker 发布链路继续收口
- **发布脚本现在优先复用现有 Docker 登录态**: `scripts/deploy/auto-update-docker.sh` 不再强制每次都导出 `DOCKERHUB_TOKEN / GHCR_TOKEN`；本机已经 `docker login` 过时，可以直接继续发多架构镜像。
- **新增“发镜像后顺手滚动服务器”的一体脚本**: `scripts/deploy/publish-and-rollout.sh` 会先调用 Docker 发布脚本，再按目标列表通过 `expect + ssh` 依次执行远端安装或升级，适合固定维护 `10.31.1.254` 与 `10.31.2.242` 这类多机多栈环境。

### 📦 默认镜像与部署模板统一到新版本
- **部署模板不再回落到旧版 `4.5.55`**: `deploy/docker-compose.yml`、`deploy/.env.example`、`fresh-install.sh`、`update-app.sh`、`update-agent.sh`、`verify-stack.sh` 等默认主程序镜像已统一切到 `v4.5.59`。
- **离线包与构建脚本默认版本一并收口**: `deploy/scripts/build-all-and-push.sh`、`deploy/scripts/export-offline-packages.sh`、GitHub Actions 工作流默认版本和 README 示例版本都已经同步抬升到 `v4.5.59`。

### 📚 文档与 Release 复核口径补齐
- **README / 部署文档 / 帮助中心同步补齐最短主线**: 文档新增 Docker Compose 首装、代码更新后直接发镜像、以及双机多实例滚动升级示例，后续照着 README 就能复用相同节奏继续迭代。
- **Release 附件巡检默认范围更新到最近版本**: `scripts/utils/check-release-assets.js` 的默认巡检范围已调整到 `v4.5.54 ~ v4.5.59`，便于继续核对当前版本和最近几个正式版本的附件完整性。

## [v4.5.58] - 农场批量作业导航与 QQ 高风险保存预览增强版 (2026-03-31)
### 🌾 农场批量作业导航增强
- **批量模式新增分组快选与覆盖率提示**: `FarmPanel.vue` 现在提供“可收获 / 待浇水 / 待除草 / 待除虫”分组快选、覆盖率摘要、空状态引导和批量颜色图例，减少管理员在大面积土地里反复逐块挑选的成本。
- **批量执行反馈升级为可追踪的结果回路**: 新增底部批量操作坞、选择数量脉冲提示和结果自动淡出；执行批量操作后会短时高亮刚处理过的地块，便于确认“哪些地已经执行过”。
- **土地卡片补齐批量语义标签和色带**: `LandCard.vue` 在批量模式下会显示最多 3 条待办标签，并按收获/浇水/除草/除虫给出不同色带，配合批量分组更容易形成“先处理哪一批”的节奏。

### 🎒 背包详情弹窗交互稳态补强
- **详情弹窗补齐键盘与焦点可达性**: `BagPanel.vue` 新增 Esc 关闭、焦点转移与回归管理，打开后会自动聚焦详情面板，关闭后回到之前操作位置，减少键盘操作时的焦点丢失。
- **遮罩关闭与滚动锁定更一致**: 详情弹窗改为独立 shell + backdrop 结构，支持点击背景关闭、关闭按钮事件收口与 `body` 滚动锁定，避免弹窗打开时背景继续滚动或误触穿透。

### 🛡️ QQ 高风险保存前预览增强
- **设置页新增高风险保存预览卡片**: `Settings.vue` 会在保存前根据“是否新开启高风险项”“是否切到不限制时长”“当前已开启数量”等条件生成预览摘要，并在页底保存区、悬浮保存坞和差异确认弹窗三处同步提示。
- **不限制模式视觉与文案进一步区分**: QQ 高风险窗口工具条、模式切换和状态胶囊增加了“手动收口模式”强化样式与动效，帮助管理员在保存前明确区分“临时窗口”与“不自动回退”两类风险级别。

### 📦 发布口径继续统一
- **当前默认发布版本统一抬升到 `v4.5.58`**: `core/package.json`、`web/package.json`、README、部署模板、Docker 发布脚本、GitHub Actions 默认版本与帮助中心 Release Notes 已统一切到 `v4.5.58`。
- **本轮会继续承接 GitHub Release / Docker Hub / 服务器升级主线**: 这次版本作为新的对外正式版本，用于 GitHub Releases、Docker Hub 镜像推送、更新公告追加与多端口部署验证。
