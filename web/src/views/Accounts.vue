...
type StateFilter = 'all' | 'online' | 'starting' | 'offline' | 'banned'
...
const ACCOUNT_BANNED_ERROR_CODE = 1000016
...
function normalizeStateFilter(value: unknown): StateFilter {
  const raw = String(value || '').trim()
  if (raw === 'online' || raw === 'starting' || raw === 'offline' || raw === 'banned')
    return raw
  return 'all'
}
...
function resolveRuntimeState(acc: any) {
  ...
  if (isAccountBanned(acc)) {
    return {
      section: 'banned' as const,
      label: '已封禁',
      dot: 'accounts-runtime-dot-banned',
      badge: 'accounts-badge-runtime-banned',
    }
  }
  return {
...
function getAccountOperationSets(acc: any) {
  ...
  const runtimeAction = getRuntimeActionMeta(acc)
  ...
  return {
    hero: createActionButtons([
      createActionButton({
        ...
        iconClass: runtimeAction.iconClass,
        variant: 'secondary',
        size: 'sm',
        block: true,
...
function getAccountDisplayFields(acc: any) {
  ...
  return {
    ...
    state: createRecordDisplayField({
      ...
      noteClass: acc.wsError?.message || isAccountBanned(acc) ? 'accounts-error-banner px-3 py-2' : 'glass-text-muted',
...
function getRuntimeActionButtonClasses(acc: any) {
  if (isAccountBanned(acc))
    return 'accounts-runtime-button accounts-runtime-button-banned'
  return acc.running || acc.connected
    ? 'accounts-runtime-button accounts-runtime-button-stop'
    : 'accounts-runtime-button accounts-runtime-button-start'
}

function getAccountWsErrorMessage(acc: any) {
  return String(acc?.wsError?.message || '').trim()
}

function getAccountBannedReason(acc: any) {
  return getAccountWsErrorMessage(acc) || '账号已被封禁'
}

function isAccountBanned(acc: any) {
  const wsErrorCode = Number(acc?.wsError?.code || 0)
  const wsErrorMessage = getAccountWsErrorMessage(acc)
  return wsErrorCode === ACCOUNT_BANNED_ERROR_CODE || /已被封禁|账号已被封禁|封禁/.test(wsErrorMessage)
}

function getRuntimeActionMeta(acc: any) {
  if (isAccountBanned(acc)) {
    return {
      label: '已封禁',
      iconClass: 'i-carbon-warning-alt-filled',
      disabled: true,
      title: `该账号已被服务端封禁，已阻止启动。原因：${getAccountBannedReason(acc)}`,
    }
  }
  return acc.running || acc.connected
    ? {
        label: '停止',
        iconClass: 'i-carbon-stop-filled',
        disabled: false,
        title: '停止账号',
      }
    : {
...
function getAccountStatusDetail(acc: any) {
  if (isAccountBanned(acc))
    return `服务端已拒绝该账号登录，建议保持停用。原因：${getAccountBannedReason(acc)}`
...
const bannedSelectedAccounts = computed(() => selectedAccounts.value.filter(acc => isAccountBanned(acc)))
const startableSelectedAccounts = computed(() => selectedAccounts.value.filter(acc => !acc?.running && !isAccountBanned(acc)))
const stoppableSelectedAccounts = computed(() => selectedAccounts.value.filter(acc => !!acc?.running))
const restartableSelectedAccounts = computed(() => selectedAccounts.value.filter(acc => !isAccountBanned(acc) && (!!acc?.running || !!acc?.connected)))
...
async function batchToggleAccounts(action: 'start' | 'stop') {
  ...
  if (targetAccounts.length === 0) {
    if (action === 'start' && bannedSelectedAccounts.value.length > 0) {
      toast.warning('所选账号里没有可启动项，已封禁账号已被自动跳过')
      return
    }
...
async function batchRestartAccounts() {
  if (restartableSelectedAccounts.value.length === 0) {
    if (bannedSelectedAccounts.value.length > 0) {
      toast.warning('所选账号里没有可重启项，已封禁账号已被自动跳过')
      return
    }
...
async function toggleAccount(account: any) {
  if (isAccountBanned(account)) {
    toast.warning(`该账号已被服务端封禁，已阻止启动。原因：${getAccountBannedReason(account)}`)
    return
  }
  if (account.running) {
...
.accounts-runtime-dot-banned {
  background: var(--ui-status-danger) !important;
}
...
.accounts-runtime-button-banned {
  border-color: color-mix(in srgb, var(--ui-status-danger) 26%, transparent) !important;
  background: linear-gradient(
    135deg,
    color-mix(in srgb, var(--ui-status-danger) 12%, transparent),
    color-mix(in srgb, var(--ui-status-danger) 18%, transparent)
  ) !important;
  color: color-mix(in srgb, var(--ui-status-danger) 84%, var(--ui-text-1)) !important;
